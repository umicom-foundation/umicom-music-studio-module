...
int main(void)
{
    ...
    const UmiExperienceLayoutDefinition *layout =
        umi_music_studio_workspace_default();
    const UmiExperienceFeatureDefinition *next =
        umi_music_studio_workspace_next_feature();

    if (definition == NULL ||
        umi_music_studio_application_status(&status) != UMI_STATUS_OK ||
        layout == NULL) {
        fputs("Umicom Music Studio: Framework experience unavailable\n", stderr);
        return 1;
    }
