...
int main(void)
{
    ...
    assert(strcmp(adoption->application_id, "org.umicom.music-studio") == 0);
    assert(umi_product_application_adoption_validate(adoption) ==
           UMI_STATUS_OK);
    assert(umi_music_productisation_snapshot(&snapshot) ==
           UMI_STATUS_OK);
    ...
    assert(umi_music_product_session_init(&session) ==
           UMI_STATUS_OK);
    ...
    assert(umi_music_product_workspace_guide(&workspace_guide) ==
           UMI_STATUS_OK);
